...
from dehaze import DepthNet
from dehaze.UNet import UNet
...
class DIACMPN(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, window_size=8,
        ...
        self.conv1 = nn.Conv2d(193,96,1)
        self.conv2 = nn.Conv2d(288,192,1)
        self.conv3 = nn.Conv2d(576,384,1)
        self.conv4 = nn.Conv2d(288,192,1)
        self.conv5 = nn.Conv2d(192,96,1)

        self.conv11 = nn.Conv2d(1, 1, kernel_size=3, stride=2, padding=1)
        self.conv22 = nn.Conv2d(1, 1, kernel_size=3, stride=4, padding=1)

        self.Unet1 = UNet(n_channels=in_chans, n_classes=out_chans)
        self.d3_conv= nn.Conv2d(in_chans,96,1)
        self.d2_conv = nn.Conv2d(128, 96, 1)
        self.d1_conv = nn.Conv2d(256, 192, 1)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.ca = nn.Sequential(
            nn.Conv2d(672, 256, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 3, 1, padding=0, bias=True),
        )
        self.fuse_conv1 = nn.Sequential(nn.Conv2d(672, 96, kernel_size=3, stride=1, padding=1),
                                   nn.InstanceNorm2d(24),
                                   nn.ReLU(True))
        self.fuse_conv2 = nn.Sequential(nn.Conv2d(672, 192, kernel_size=3, stride=2, padding=1),
                                        nn.InstanceNorm2d(24),
                                        nn.ReLU(True))
        self.fuse_conv3 = nn.Sequential(nn.Conv2d(672, 384,  kernel_size=5, stride=4, padding=1),
                                        nn.InstanceNorm2d(24),
                                        nn.ReLU(True))
        self.conv_d1 = nn.Conv2d(768, 384, 1)
        self.conv_d2 = nn.Conv2d(384, 192, 1)

        self.conv_fu = nn.Conv2d(97, 96, 1)
...
if __name__ == '__main__':
    ...
    net = DIACMPN(in_chans=100, out_chans=100, embed_dims=[96, 192, 384, 192, 96]).to(device)
