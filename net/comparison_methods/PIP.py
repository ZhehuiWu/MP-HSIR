...
import sys
sys.path.append("..")
from comparison_methods.PIP_utils import *
...
class Basic_prompt(nn.Module):
    ...
    def _unify_sementic_prompt_shape(self, origin_prompt, opt = None):
        ...
        if opt == 'task':
            ...
            return self.clip_linear(origin_prompt).unsqueeze(-1).unsqueeze(-1)
...
class PromptInPrompt(Basic_prompt):
    ...
    def __init__(self, prompt_dim = 64, 
        ...
        # 【1】 prompt type
        self.use_detask_prompt, self.use_CLIP_prompt = use_detask_prompt, use_CLIP_prompt
        ...
        self.detask_prompt_param = nn.Parameter(torch.randn(1, task_classes, low_prompt_dim, 1, 1))  ##1,4,96,1,1
        # 【1.2】 basic restoration prompt
        self.low_prompt_param = nn.Parameter(torch.randn(1,low_prompt_dim,prompt_size,prompt_size))  ## 1,96,128,128

        # 【2】 CLIP prompts -- provide sementic prompt
        ...
        # prompt interaction
        self.LGM = CrossTransformerRestormer_BlockV2(dim = low_prompt_dim, num_heads = 2, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias')
    ...
    def forward(self, x, degradation_class = None):
        ...
        if degradation_class.ndimension() > 1:
        ...
        output_prompt = self.outconv(output_prompt) #1,96,128,128
...
class PromptInPrompt2(Basic_prompt):
    '''
    '''
    def __init__(self, 
                task_classes = 13, 
        ...
        super(PromptInPrompt2,self).__init__()
        
        self.task_text_prompt = [
            "A hyperspectral image with blur and Gaussian noise.",
            "A hyperspectral image with blur, Gaussian noise, and deadline noise.",
            "A hyperspectral image with blur, Gaussian noise, and impulse noise.",
            "A hyperspectral image with blur, Gaussian noise, and stripe noise.",
            "A hyperspectral image with super-resolution and Gaussian noise.", 
            "A hyperspectral image with super-resolution, Gaussian noise, and deadline noise.",
            "A hyperspectral image with super-resolution, Gaussian noise, and impulse noise.",
            "A hyperspectral image with super-resolution, Gaussian noise, and stripe noise.",
            "A hyperspectral image with blur, super-resolution and Gaussian noise",
            "A hyperspectral image with blur, super-resolution, Gaussian noise, and deadline noise",
            "A hyperspectral image with blur, super-resolution, Gaussian noise, and impulse noise",
            "A hyperspectral image with blur, super-resolution, Gaussian noise, and stripe noise",
            "A hyperspectral image with inpaint and Gaussian noise."
        ]
        ...
        # 【1.1】degradation-aware prompt
        self.detask_prompt_param = nn.Parameter(torch.randn(1, task_classes, text_dim//2, 1, 1))  ##1,4,96,1,1
        # 【1.2】 basic restoration prompt
        self.low_prompt_param = nn.Parameter(torch.randn(1,low_prompt_dim,prompt_size,prompt_size))  ## 1,96,128,128

        #【2】 CLIP prompts -- provide sementic prompt
        if self.use_CLIP_prompt:
            # self.clip_conv = nn.Conv2d(512, low_prompt_dim//2 , 1, 1, 0)
            self.clip_linear = nn.Linear(512, text_dim//2)
            self.clip_prompt = self._clip_preprocess()
        
        # 【4. universal restoration prompt by prompt in prompt learning】
        # prompt interaction
        self.LGM = CrossTransformerRestormer_BlockV2(dim = low_prompt_dim, num_heads = 2, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias')
        # self.norm1 = LayerNorm(prompt_dim, 'WithBias')
        # self.cross_attn = Cross_Attention(prompt_dim, 6, False)
        # 【5. output layer】 reduce channel from 2*dim -> 1*dim
        self.outconv = nn.Conv2d(low_prompt_dim * 1, 
                                 low_prompt_dim, 
                                 kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x, degradation_class = None):
        ...
        #         pass
        #     sementic_prompt = sementic_prompt * feature
            sementic_prompt = F.interpolate(sementic_prompt, size=(self.prompt_size, self.prompt_size))
            #1,96,128,128
            low_prompt = self.LGM(sementic_prompt,self.low_prompt_param.repeat(B, 1, 1, 1))
            # low_prompt = self.cross_attn(self.norm1(self.low_prompt_param.repeat(B, 1, 1, 1)),sementic_prompt)
        else:
            raise ValueError("NOT THE CURRENT VERSION")
...
class PromptToFeature(nn.Module):
    ...
    '''
    def __init__(self, 
                interaction_mode = 'pip_cross_topm',
                interaction_opt = {
        'feat_dim': 192, 'prompt_dim': 192, 'head': 6, 
...
class PromptToFeature2(nn.Module):
    def __init__(self, 
                interaction_mode = 'pip_cross_topm',
                interaction_opt = {
        'feat_dim': 192, 'prompt_dim': 192, 'head': 6, 
...
if __name__ == '__main__':
    ...
    x = torch.rand((1, 96, 128, 128)).to(device)
    # y = torch.randint(low=0, high=4, size=(1, 1)).to(device)
    y = torch.tensor([(1, 2,3)]).to(device)
 
    #t = torch.randint(0, 1000, (1,), device=device).long()
    net = PromptInPrompt2().to(device)
